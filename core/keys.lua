...
events.connect(events.KEY, function(code, modifiers)
	...
	-- Since printable characters are uppercased, disable shift. (Disable for non-characters on Haiku)
	if (not HAIKU) then 
		if shift and code >= 32 and code < 256 then shift = false end 
	else
		if shift and ((code >= 48 and code < 56) or (code >= 65 and code < 90) or (code >= 97 and code < 122)) then shift = false end 
	end
